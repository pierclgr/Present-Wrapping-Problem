...
def plot_solution(input_instance, corners, rotation):
    n_presents = input_instance['n_presents']
    cmap = plt.cm.get_cmap('hsv', n_presents + 1)
    fig1 = plt.figure("Solution", figsize=(15, 9))
    ax1 = fig1.add_subplot(111, aspect='equal')
    ax1.set_title("Solution to " + str(input_instance['paper_width']) + "x" + str(input_instance['paper_height']))
    ax1.set_xlabel('x')
    ax1.set_ylabel('y')
    rectangles = []
    for i in range(n_presents):
        if rotation[i]:
            rectangle = Rectangle((corners[i][0], corners[i][1]), input_instance['presents_dimensions'][i][1],
                                  input_instance['presents_dimensions'][i][0], color=cmap(i), ec='black', lw=1.5,
                                  clip_on=False, label=str(input_instance['presents_dimensions'][i][0]) + "x" +
                                                       str(input_instance['presents_dimensions'][i][1]) + " (rotated)")
        else:
            rectangle = Rectangle((corners[i][0], corners[i][1]), input_instance['presents_dimensions'][i][0],
                                  input_instance['presents_dimensions'][i][1], color=cmap(i), ec='black', lw=1.5,
                                  clip_on=False, label=str(input_instance['presents_dimensions'][i][0]) + "x" +
                                                       str(input_instance['presents_dimensions'][i][1]))
        rectangles.append(rectangle)
        point = plt.Circle((corners[i][0], corners[i][1]), 0.2, color='black', clip_on=False)
        ax1.add_patch(rectangle)
        ax1.add_artist(point)
    ax1.add_artist(
        Rectangle((0, 0), input_instance['paper_width'], input_instance['paper_height'], fill=False, ec='black', lw=1.5,
                  clip_on=False))
    plt.ylim((0, input_instance['paper_height']))
    ax1.set_yticks(np.arange(0, input_instance['paper_height'] + 1, 1))
    plt.xlim((0, input_instance['paper_width']))
    ax1.set_xticks(np.arange(0, input_instance['paper_width'] + 1, 1))
